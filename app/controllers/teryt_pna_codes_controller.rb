class TerytPnaCodesController < ApplicationController
  ...
  def index
    @teryt_pna_codes = TerytPnaCode.all
  end

  def select2_index
    params[:q] = params[:q]
    @teryt_pna_codes = TerytPnaCode.order(:mie_nazwa, :uli_nazwa, :pna).finder_teryt_pna_code(params[:q])
    @teryt_pna_codes_on_page = @teryt_pna_codes.page(params[:page]).per(params[:page_limit])

    respond_to do |format|
      format.html
      format.json { render json: @teryt_pna_codes_on_page, each_serializer: TerytPnaCodeSerializer, meta: {total_count: @teryt_pna_codes.count} } 
    end
  end
  ...
  def new
    @teryt_pna_code = TerytPnaCode.new
  end
  ...
  def create
    @teryt_pna_code = TerytPnaCode.new(teryt_pna_code_params)
  ...
  private
    ...
    def set_teryt_pna_code
      @teryt_pna_code = TerytPnaCode.find(params[:id])
    end
