class AddProposalViaInternetToDivision < ActiveRecord::Migration
...
#  DIVISION_R_FOR_SHOW = [ DIVISION_R_A, DIVISION_R_C ]  
  ...
  def up
    add_column :divisions, :proposal_via_internet, :boolean, null: false, default: true

    Division.where(id: DIVISION_EXCLUDE_FOR_INTERNET ).update_all(proposal_via_internet: false)
  end

  def down
    remove_column :divisions, :proposal_via_internet
  end
end