...
class PdfProposalCardsR < Prawn::Document

  def initialize(proposals, exam, view)
    ...
    count_table_rows = @proposals.length

    @proposals.each_with_index do |proposal, i|
      data(proposal, i)
#      display_data_table(proposal)
  ...
  def display_data_table(proposal)
    bounding_box([0, 600], :width => 525, :height => 400) do 
      if table_data(proposal).empty?
        text "No Events Found"
      else
        table( table_data(proposal),
              :header => true,
      ...
      display_total_table(proposal)
    end  
  end


  def table_data(proposal)
    table_data ||= [["Lp",
                     "Przedmiot",
                     "Ocena (słownie)",
                     "Podpis egzaminatora"]] + 
                     proposal.grades.order(:id).map { |p| [ 
                        p.subject.item, 
                        p.subject.name,
                        "",
                        ""
  ...
  def display_total_table(e)
    if Esodes::CORRECTION_EXAMINATIONS.include?(e.esod_category)
      customer_last_proposal = Examination.where(customer: e.customer, division: e.division, proposal_result: 'N').last
      move_down 10
      text "Egzamin powtórny. Poprzednia sesja egzaminacyjna: " + "#{customer_last_proposal.exam.number} - wynik negatywny." if customer_last_proposal.present?
    end  
  end
  ...
  def data(proposal, i)
    ...
    move_down 20
    text "#{proposal.customer.fullname}", size: 11, :style => :bold 
    move_down 5
    text "ubiega się o #{proposal.division.name}"
  end
